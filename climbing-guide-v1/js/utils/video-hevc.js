/**
 * Lightweight MP4/MOV HEVC detection by walking boxes for stsd codecs.
 * Works on a full buffer or head+tail slices (moov usually at start or end).
 */
 ...
 * @param {number} start
 * @param {number} end
 * @param {(type: string, contentStart: number, contentEnd: number) => boolean | void} visit
...
/** True when this MP4 buffer's sample description lists an HEVC codec. */
export function mp4BufferHasHevc(bytes) {
  ...
  // Fallback: codec fourcc near box headers (covers truncated head/tail slices).
 ...
 * @param {{ maxScanBytes?: number }} [options]
 */
export async function fileLooksLikeHevc(file, { maxScanBytes = 4 * 1024 * 1024 } = {}) {
