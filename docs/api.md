...
| `POST` | `/api/pins/:pinId/thumbnail` | allowlisted | multipart `mapId` + `file` (max 2MB), or JSON `{ mapId, thumbnailUrl }` — fill-if-empty compact still; `200` `{ thumbnail, pin, alreadySet }` |
...
Common errors: `400` validation (including expired Discord links — *"Discord link expired — copy a fresh attachment URL from Discord or upload the file directly"*), `403` role/ownership, `404` pin not found, `498` expired detail token, `503` KV or R2 unavailable.
